...
public class AprobarSolicitudCommandHandlerTests
{
    ...
    [Fact]
    public async Task HandleAsync_DatosValidos_MueveSaldoDePendienteAConsumido()
    {
        // Arrange
        ...
        // Act
        await handler.HandleAsync(new AprobarSolicitudCommand(solicitud.Id, aprobadorId), CancellationToken.None);
        ...
        await _histRepo.Received(1).AgregarAsync(
            Arg.Is<HistorialSolicitud>(h => h.Actor == aprobador.Email), Arg.Any<CancellationToken>());
        await _uow.Received(1).SaveChangesAsync(Arg.Any<CancellationToken>());
    }

    [Fact]
    public async Task HandleAsync_MismoEmpleadoQueAprueba_LanzaAutoAprobacionNoPermitidaException()
    {
        // Arrange
        var empleadoId = Guid.NewGuid();
        var solicitud = CrearSolicitud(empleadoId);
        ...
        await Assert.ThrowsAsync<AutoAprobacionNoPermitidaException>(() =>
            handler.HandleAsync(new AprobarSolicitudCommand(solicitud.Id, empleadoId), CancellationToken.None));
    }
    ...
    public async Task HandleAsync_AprobadorInactivo_LanzaAprobadorInactivoException()
    {
        ...
        await Assert.ThrowsAsync<AprobadorInactivoException>(() =>
            handler.HandleAsync(new AprobarSolicitudCommand(solicitud.Id, aprobadorId), CancellationToken.None));
    }
    ...
    public async Task HandleAsync_SolicitudNoEncontrada_LanzaSolicitudNoEncontradaException()
    {
        ...
        await Assert.ThrowsAsync<SolicitudNoEncontradaException>(() =>
            handler.HandleAsync(new AprobarSolicitudCommand(solicitudId, Guid.NewGuid()), CancellationToken.None));
    }
